# """
# ================================================================================
#  Standalone Playwright crawler — ASPX-serving regulator sites (ZATCA, MC, ...)
# ================================================================================
# """
...
() => {
  ...
  // MC-specific: if this page has the Law/Regulation/Attachments accordion
  // structure (confirmed markup: mc.gov.sa law detail pages -- .rule-
  // container, .regulation-container, .attachment-container as direct
  // siblings under #lawItemContainer), also return each panel's own html/
  // text separately, on top of the whole-page html/text already returned
  // below. Purely ADDITIVE: the existing html/text keys are completely
  // unchanged and still cover the whole page for every site. This extra
  // mc_parts key is only ever present when these exact three containers
  // exist in the cleaned clone, so it has zero effect on any other site's
  // output, and zero effect on MC pages that aren't a law detail page
  // (e.g. the listing page itself).
  const lawPanel = clone.querySelector('.rule-container');
  const regPanel = clone.querySelector('.regulation-container');
  const attPanel = clone.querySelector('.attachment-container');
  let mcParts = null;
  if (lawPanel || regPanel || attPanel) {
    mcParts = {};
    if (lawPanel) mcParts.law = { html: lawPanel.innerHTML, text: (lawPanel.innerText || '').trim() };
    if (regPanel) mcParts.regulation = { html: regPanel.innerHTML, text: (regPanel.innerText || '').trim() };
    if (attPanel) mcParts.attachment = { html: attPanel.innerHTML, text: (attPanel.innerText || '').trim() };
  }

  return { html: clone.innerHTML, text: (clone.innerText || '').trim(), mc_parts: mcParts };
}
"""
...
def crawl(seed_url, out_dir, max_pages=150, max_depth=8, scope="breadcrumb",
    ...
    section_anchor = None
    used_slugs = set()
    used_law_slugs = set()   # MC-only: per-law folder names (see mc_parts below),
                              # tracked separately from used_slugs since it names
                              # folders, not html files, and is keyed on the
                              # law's own title rather than the page's URL path
    ...
    with sync_playwright() as pw:
        ...
        while (queue or page_queue) and len(records) < max_pages:
            ...
            # MC-specific split: one folder per law, with Law/Regulation/
            # Attachments as three separate files and three separate
            # records, instead of one combined file. Gated on BOTH seed_host
            # AND the presence of mc_parts (belt-and-suspenders -- mc_parts
            # only appears when the specific .rule-container/.regulation-
            # container/.attachment-container markup was actually found, so
            # this can't misfire even in theory on a page that merely shares
            # a host with mc.gov.sa). Every other site, and every MC page
            # that ISN'T a law detail page (e.g. the listing page), falls
            # straight through to the unchanged single-file path below.
            mc_parts = content.get("mc_parts") if seed_host == "mc.gov.sa" else None
            if mc_parts:
                law_slug_base = slugify(title) or slug_base
                law_slug = law_slug_base
                if law_slug in used_law_slugs:
                    suffix = hashlib.md5(url.encode()).hexdigest()[:8]
                    law_slug = f"{law_slug_base}-{suffix}"
                used_law_slugs.add(law_slug)
                law_dir = Path("mc") / law_slug
                (out / law_dir).mkdir(parents=True, exist_ok=True)

                for part_name, part_content in mc_parts.items():
                    part_html_file = str(law_dir / f"{part_name}.html")
                    (out / part_html_file).write_text(part_content["html"], encoding="utf-8")
                    part_rec = {
                        "section_path": " > ".join(breadcrumb),
                        "title": title,
                        "part": part_name,          # law | regulation | attachment
                        "url": url,
                        "depth": depth,
                        "linked_from_title": link_titles.get(url, ""),
                        "parent_page_url": link_parents.get(url, ""),
                        ...
                        "n_pdfs": len(page_docs),
                        "pdf_links": " | ".join(page_docs),
                        "text_len": len(part_content["text"]),
                        "html_file": part_html_file,
                        "text": part_content["text"],
                        "html": part_content["html"],
                        "breadcrumb": breadcrumb,
                    }
                    records.append(part_rec)
                emit({"event": "visit", "url": url, "depth": depth, "title": title,
                      "section_path": " > ".join(breadcrumb), "n_pdfs": len(page_docs),
                      "mc_parts": list(mc_parts.keys()), "recorded": len(records),
                      "queued": len(queue), "page_queued": len(page_queue)})
                # Skip the single-file write and the generic rec/emit below --
                # the three part-records above already cover this page.
                if depth < max_depth:
                    for l in links:
                        href = l["href"]
                        nh = normalize_url(href)
...
def _write_excel(path, records, documents):
    ...
    page_rows = [{
        "section_path": r["section_path"],
        "title": r["title"],
        "part": r.get("part", ""),   # MC only: law | regulation | attachment; blank for every other record
        "url": r["url"],
        "depth": r["depth"],
        "linked_from_title": r.get("linked_from_title", ""),
        "parent_page_url": r.get("parent_page_url", ""),
