...
EXTERNAL_LAW_PORTALS = {
    ...
    "mc.gov.sa",                        # Ministry of Commerce regulations
    "moj.gov.sa", "laws.moj.gov.sa",   # Ministry of Justice
    "pr.gov.sa",                        # Premium Residency
    "zatca.gov.sa",                     # Tax & Customs Authority
}
def is_external_law_portal(url: str) -> bool:
    """True if a URL's host is a known Saudi government legal portal that
    hosts law text itself, rather than a document extension/path pattern.
    Used to recognize links like laws.boe.gov.sa/BoeLaws/Laws/LawDetails/...
    as documents, even though they carry no .pdf/.docx extension."""
...
def is_document_link(url: str) -> bool:
    """True if a link points to a downloadable document — not just plain
    .pdf/.docx, but also download-manager links (WordPress Download Manager
    `wpdmdl=`, /document/, /download/ endpoints) that serve a file without a
    file extension in the URL, AND known external law-portal domains that
    host law text at a URL with no file extension at all (e.g. BOE, MOJ)."""
...
() => Array.from(document.querySelectorAll('a[href]')).map(a => {
  ...
  // Chrome = is this link inside a semantic header/footer landmark? Deliberately
  // NOT excluding <nav>/[role="navigation"] here -- unlike header/footer, <nav>
  // is sometimes legitimately used for in-page content navigation (e.g. a
  // sidebar of categories), which we don't want to accidentally treat as chrome
  // on a site we haven't seen yet. header/footer are reliable signals; nav isn't.
...
def crawl(seed_url, out_dir, max_pages=150, max_depth=8, scope="breadcrumb",
    ...
    link_parents = {}   # normalized url -> the page it was found on
    ...
    with sync_playwright() as pw:
        ...
        while (queue or page_queue) and len(records) < max_pages:
            ...
            if not title:
                title = (page.title() or "").strip()
            # ---compute nav_path per link on this page ---
            try:
            ...
            for l in links:
                href = l["href"]
                if l.get("chrome"):   # skip header/footer/nav landmarks entirely
                    continue
                if urlparse(href).scheme in ("http", "https") and is_document_link(href):
                    ...
                    sp = " > ".join(breadcrumb)
                    section_path = f"{sp} > {nav}" if nav else sp
