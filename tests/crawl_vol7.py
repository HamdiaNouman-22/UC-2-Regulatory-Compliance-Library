...
crawl_vol7.py
=============
Crawls only CBB Rulebook Vol 7 (Collective Investment Undertakings),
saves Excel for review and JSON cache for push_vol7_to_db.py.

Usage:
    python tests/crawl_vol7.py
"""
import sys
import logging
import json
...
from crawl_to_excel import crawl_volume, save_to_excel, _collect_volumes, _clean_html, _html_to_text
...
def main():
    ...
    if not vol7:
        log.error("Vol 7 not found. Available:")
        for v in volumes:
            log.info(f"  - {v.text}")
        return
    ...
    # Save cache as vol7_pending.json (expected by push_vol7_to_db.py)
    ...
    log.info("")
    log.info("=" * 60)
    log.info(f"Review Excel at: {excel_path.resolve()}")
    log.info("If correct, run: python tests/push_vol7_to_db.py")
    log.info("=" * 60)
